...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            // 1. Coverting the string constant to a JSON Object
            JSONObject jQuakes = new JSONObject(SAMPLE_JSON_RESPONSE);

            // 2. Extracting "features" JSON array
            JSONArray features = jQuakes.getJSONArray("features");

            // 3. loop through each feature in the array, which includes data for multiple quakes
            for (int i = 0; i < features.length(); i++)
            {
                JSONObject currentQuake = features.getJSONObject(i);  // narrowing down to an individual quake
                // 4. Now get the "properties" JSON Object
                JSONObject properties = currentQuake.getJSONObject("properties");
                // 5. extract "mag", "place", and "time"
                // Extract the value for the key called "time" long time = properties.getLong("time");
               String magnitude = properties.getString("mag");
               String location = properties.getString("place");
               long time = properties.getLong("time");

               // 6. Create an Earthquake object from magnitude, location, and time
                Earthquake earthquake = new Earthquake(magnitude, location, time);

                // 7. Add this earthquake to the list of earthquakes with the ArrayList.add() method
